package com.example.chatbot.chat.response;
...
public record DocumentStatusResponse(
    ...
    boolean cancelRequested  // 취소 요청 후 아직 완전히 멈추지 않은 상태인지 여부
) {
    // 기존 생성자에 대한 호환성 유지
    public DocumentStatusResponse(boolean processing, int processedCount, int totalCount) {
        this(processing, processedCount, totalCount, 0, totalCount > 0, false);
    }

    public DocumentStatusResponse(boolean processing, int processedCount, int totalCount, int changedCount) {
        this(processing, processedCount, totalCount, changedCount, totalCount > 0, false);
    }

    public DocumentStatusResponse(boolean processing, int processedCount, int totalCount, int changedCount,
            boolean cancelRequested) {
        this(processing, processedCount, totalCount, changedCount, totalCount > 0, cancelRequested);
    }
}
