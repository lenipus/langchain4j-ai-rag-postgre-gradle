...
public interface RagChatbot {
    ...
    String RAG_SYSTEM_PROMPT = """
            ...
            - 문서 안의 표나 목록이 대상(예: 직급, 부서)별로 서로 다른 개별 규칙을 나열한 것인지,
              하나의 순차적 절차를 나열한 것인지 문서 내용을 보고 정확히 구분하세요. 문서에
              순서/절차라고 명시돼 있지 않다면, 여러 항목을 근거 없이 하나로 이어붙이지 마세요.
              (예: "직급별 결재자" 표는 질문자의 직급에 해당하는 한 줄만 적용하는 것이지,
              모든 줄을 순서대로 이어 붙인 다단계 절차가 아닙니다.)
