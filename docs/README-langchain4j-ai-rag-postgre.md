...
# 채팅 메모리 설정 (DB는 전체 보존, 아래 값은 LLM에 보낼 히스토리에만 사용)
chat:
  memory:
    context-reserve-tokens: 12000   # 시스템 프롬프트/RAG 삽입 문서/응답 생성분 여유
    max-history-turns: 10           # 토큰 예산이 남아도 최대 이 턴(질문+답변 쌍) 수까지만
    fallback-context-tokens: 8192   # 모델 컨텍스트 길이를 알 수 없을 때 대신 쓰는 값
