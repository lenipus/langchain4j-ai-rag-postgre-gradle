...
로컬 LLM(예: `Arctic-Text2SQL-R1-7B-GGUF`)을 다룰 때 자주 접하는 **파일 포맷 → 추론 엔진 → API 규약** 3단계 구조를 정리한다. 이 프로젝트(`egovframe-ai-rag`)는 이 중 **GGUF + Ollama + Ollama REST API** 조합을 사용한다 ([[embedding-and-ollama-setup]] 참고).
