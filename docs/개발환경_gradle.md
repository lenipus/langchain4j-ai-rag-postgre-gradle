...
IntelliJ IDEA를 이용해 이 프로젝트(`egovframe-ai-rag`, Gradle 기반)의 개발 환경을 처음부터 구성하는 절차. `docs/` 아래 개별 문서들(VSCode/Eclipse/Maven 기준으로 작성된 과거 문서 포함)의 핵심 내용을 IntelliJ + Gradle 기준으로 재정리한 통합 가이드다. 더 깊은 배경/트러블슈팅은 각 절에서 링크하는 개별 문서를 참고한다.
...
- [7. 공통 인프라 기동 — Redis Stack + PostgreSQL(pgvector)](#7-공통-인프라-기동--redis-stack--postgresqlpgvector)
...
| Git | 최신 | 소스 관리 |
| Docker Desktop | 최신 | Redis Stack, PostgreSQL(pgvector) 컨테이너 구동 (7절) |
| Docker (WSL, 예: RockyLinux9) | - | Ollama 컨테이너 구동 (8절, Docker Desktop과 별개) |
| Ollama | 0.17.1 이상 (하위 버전은 CVE-2026-7482 취약점) | LLM/임베딩 서빙 |

> Community 에디션에는 Spring/Docker 전용 통합 기능(하단 `Services` 대시보드 등)이 없다. Spring Boot 앱 실행은 클래스 옆 거터의 실행(▶) 아이콘으로 하고, 도커 컨테이너 관리는 IDE 밖에서 직접 한다 — Redis/PostgreSQL은 Docker Desktop(7절), Ollama는 WSL 터미널(8절)로 각각 다르게 관리한다.
...
rootProject.name = 'egovframe-ai-rag'
```
...
실제로 완성된 `build.gradle` (이 프로젝트의 현재 파일, 참고용):
```groovy
plugins {
    id 'java'
...
group = 'com.example'
version = '1.0.0'
description = 'eGovFrame AI RAG application'
...
2. **실행**: `src/main/java/com/example/chat/SpringAiOllamaApplication.java` 를 열고, 클래스 옆 거터의 ▶ 아이콘 클릭 → `Run 'SpringAiOllamaApplication'`
...
## 7. 공통 인프라 기동 — Redis Stack + PostgreSQL(pgvector)

이 절은 **Docker Desktop** 기준이다. Docker Desktop을 설치하면 Docker Compose(`docker compose` 명령)가 기본 포함되어 있어 별도 설치가 필요 없다.

Redis Stack과 PostgreSQL(pgvector)은 **둘 다 벡터 저장소로 쓸 수 있는 대안 관계**다. 이 프로젝트는 둘 중 하나만 골라서 벡터스토어로 연결하도록 되어 있으므로, 본인이 실제로 쓸 벡터DB 하나만 설치하면 된다 (둘 다 설치할 필요 없음).
...
| 역할 | 모델 | 실행 방식 |
|---|---|---|
| 임베딩 (Retrieval) | `embeddinggemma:300m` | Ollama가 서빙 → Spring AI `OllamaEmbeddingModel`이 REST API로 호출 |
| 답변 생성 (Generation) | `qwen3:4b-q4_K_M` | Ollama가 서빙 → Spring AI `OllamaChatModel`이 REST API로 호출 |
...
1. Redis Stack, Ollama 기동 확인
2. 애플리케이션 실행 → `http://localhost:8080` (또는 `SERVER_PORT`로 지정한 포트) 접속
...
- **Ollama 자체 API** (`/api/generate`, `/api/chat`): Ollama 고유 REST 규약. 이 프로젝트의 Spring AI `OllamaChatModel` / `OllamaEmbeddingModel`이 내부적으로 이 API를 호출
...
GGUF 모델 파일
  ...
  → Ollama REST API (/api/chat, /api/generate)
  → Spring AI OllamaChatModel / OllamaEmbeddingModel이 호출
...
3. `application.yml`의 `spring.ai.ollama.chat.model` (또는 `embedding.model`) 값을 새 모델 이름으로 변경
